...
// File contracts/integrations/lib/FixedPoint96.sol
...
// File contracts/strategy/Math.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/interfaces/IdcToken.sol
...
// File contracts/integrations/perpetual/IVault.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/integrations/perpetual/IExchange.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/integrations/uniswap/IUniswapV3SwapCallback.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/integrations/uniswap/IV3SwapRouter.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/integrations/perpetual/IIndexPrice.sol
// License-Identifier: GPL-2.0-or-later
...
// File contracts/integrations/perpetual/IClearingHouse.sol
// License-Identifier: GPL-2.0-or-later
...
// File @openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol@v4.4.0
// License-Identifier: MIT
...
// File contracts/strategy/StrategyStorage.sol
// License-Identifier: MIT
...
// File contracts/strategy/DeltaNeutralPerp.sol
// License-Identifier: MIT
...
contract DeltaNeutralPerp is StrategyStorage {
   ...
   // uint32 internal _subscriptionFee; // fee charged once on deposit; 1e6
    ...
    mapping(address => bool) private _allowList; // casual users
    mapping(address=>bool) private _daoMembersList; // daoMembers
    mapping(address => uint256) private _depositAmount; // todo для всех: - и тех и тех;
    ...
    */
    function adjustPosition(bool operationType, bool positionType, uint256 amount) public onlyTrigger() {
        ...
        if (operationType) { // openPosition
            uint256 usdcAmountForPerp = _calculateAmountToPosition(amount, _SHORT_VALUE);
            uint256 usdcAmountForUniswap = _calculateAmountToPosition(amount, _W_TOKEN_VALUE);
            /// short/buy wToken
            if(positionType) {
                require(getReserve() >= amount, "DNP: Insufficient reserve");
                //Remove this line on new deploy(NOT update). Approve already happened in init
                IERC20(_usdToken).approve(_router, usdcAmountForUniswap);
                _buyTokensToUniswap(_usdToken, _wToken, usdcAmountForUniswap);
                _depositToVault(usdcAmountForPerp);
                _openPosition(usdcAmountForPerp * 1e12, positionType);
                emit PositionAdjusted(operationType,
                    positionType,
                    usdcAmountForPerp,
                    usdcAmountForUniswap);
            } else {
                /// long/sell wToken
                if (Math.mulDiv(IERC20(_wToken).balanceOf(address(this)),
                    _nativeStrategyTokenPrice(0), 1e18) < usdcAmountForUniswap) {
                    _fullClose();
                } else {
                    _sellTokensToUniswap(_wToken, _usdToken, usdcAmountForUniswap,
                        IERC20(_wToken).balanceOf(address(this)));
                    _openPosition(usdcAmountForPerp, positionType);
                    _withdrawFromPerp(usdcAmountForPerp);
                    emit PositionAdjusted(operationType,
                        positionType,
                        usdcAmountForPerp,
                        usdcAmountForUniswap);
    ...
    * @return fundingRate_10_6
    ...
    function _depositToVault(uint256 amount) private {
        ...
        IERC20(_usdToken).approve(address(_perpVault), amount);
