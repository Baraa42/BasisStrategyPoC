...
contract ClearingHouseMock {
    ...
    mapping(address => int256) private accountValue;
    ...
    function openPosition(OpenPositionParams memory params) external returns (uint256 base, uint256 quote){
        if (params.isBaseToQuote) {
            accountValue[msg.sender] += int256(params.amount);
        } else {
            accountValue[msg.sender] -= int256(params.amount * 1e12);
        }
    }
...
*/
    ...
    // @dev For TESTING
    function setAccountBalance(address trader, int256 value) public {
